...
export function ProductionHistory() {
  ...
  const validatedProductions = useMemo(
    () => productionHistory.filter((record) => record.status === 'validated_by_magasinier'),
    [productionHistory]
  )

  const statistics = useMemo(() => {
    if (validatedProductions.length === 0) {
      return {
        totalProduction: 0,
        totalPieces: 0,
        averageWaste: 0,
    ...
    const totalProduction = validatedProductions.reduce((sum, r) => sum + r.usefulQuantity, 0)
    const totalPieces = validatedProductions.reduce((sum, r) => sum + r.piecesCount, 0)
    const averageWaste = validatedProductions.reduce((sum, r) => sum + r.wastePercentage, 0) / validatedProductions.length
    const averageWeightPerPiece = totalProduction / totalPieces
    const totalProductionTime = validatedProductions.reduce((sum, r) => sum + r.productionTime, 0)
    const averageSpeed = validatedProductions.reduce((sum, r) => sum + r.speed, 0) / validatedProductions.length
    ...
    return {
      ...
      totalProductionTime,
      averageSpeed,
      productionCount: validatedProductions.length,
    }
  ...
  const trendData = useMemo(() => {
    ...
    const lastFive = validatedProductions.slice(0, 5).reverse()
  ...
  const stockTrendData = useMemo(() => {
    ...
    validatedProductions.slice().reverse().forEach((record, index) => {
  ...
  const materialUsageData = useMemo(() => {
    ...
    const hdTotal = validatedProductions.reduce((sum, r) => sum + r.hdQuantity, 0)
    const ldTotal = validatedProductions.reduce((sum, r) => sum + r.ldQuantity, 0)
    const colorTotal = validatedProductions.reduce((sum, r) => sum + r.blackColorQuantity, 0)
    const dryerTotal = validatedProductions.reduce((sum, r) => sum + r.dryerQuantity, 0)
    const wasteTotal = validatedProductions.reduce((sum, r) => sum + (r.totalQuantity - r.usefulQuantity), 0)
