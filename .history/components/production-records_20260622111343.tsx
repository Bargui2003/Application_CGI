...
export function ProductionRecords() {
  ...
  // Filtrer les productions validées ou refusées par magasinier
  const filteredProductions = useMemo(() => {
    let results = [...(productionHistory || [])].filter(
      p => p.status === 'validated_by_magasinier' || p.status === 'rejected_by_magasinier'
    )
